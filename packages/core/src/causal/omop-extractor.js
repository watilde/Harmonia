...
function extractDiabetesCohort(dataset, config) {
    ...
    for (const person of dataset.persons) {
        ...
        const receivedNewDrug = drugs.some((d) => d.drug_concept_id === omop_synthetic_1.OMOP_CONCEPTS.NEW_DIABETES_DRUG &&
        ...
        const followUpMeasurement = measurements.find((m) => m.measurement_concept_id === omop_synthetic_1.OMOP_CONCEPTS.HBA1C && m.measurement_date > config.indexDate);
...
function extractICUCohort(dataset, config) {
    ...
    for (const person of dataset.persons) {
        ...
        const receivedEarlyIntervention = procedures.some((p) => p.procedure_concept_id === omop_synthetic_1.OMOP_CONCEPTS.CENTRAL_LINE &&
        ...
        const apacheScore = measurements.find((m) => m.measurement_concept_id === omop_synthetic_1.OMOP_CONCEPTS.APACHE_SCORE &&
...
function extractScreeningCohort(dataset, config) {
    ...
    for (const person of dataset.persons) {
        ...
        const procedures = proceduresByPerson.get(personId) || [];
        const receivedScreening = procedures.some((p) => p.procedure_concept_id === omop_synthetic_1.OMOP_CONCEPTS.COLONOSCOPY &&
            p.procedure_date === config.indexDate);
        ...
        const cancerDiagnosis = conditions.find((c) => c.condition_concept_id === omop_synthetic_1.OMOP_CONCEPTS.COLON_CANCER &&
