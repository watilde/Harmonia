...
function determineInferenceMode(scores, thresholds = DEFAULT_THRESHOLDS) {
  ...
  if (unconfoundedness_score >= 0.7) {
    ...
    assumptions_violated.push(
      `unconfoundedness (imbalance detected, score: ${unconfoundedness_score.toFixed(2)})`
    );
  }
  ...
  if (specification_score >= 0.7) {
    ...
    assumptions_violated.push(
      `specification (heterogeneity detected, score: ${specification_score.toFixed(2)})`
    );
  }
  ...
  return {
    ...
    assumptions_violated,
    recommendation:
      'Use Manski bounds + E-values. Report worst-case bounds and robustness metrics.',
  };
}
...
function printFederatedModeDecision(decision) {
  ...
  for (const [site_id, site_decision] of decision.site_modes.entries()) {
    console.log(
      `  ${site_id.padEnd(15)} ${site_decision.mode.padEnd(20)} (confidence: ${(site_decision.confidence * 100).toFixed(1)}%)`
    );
  }
  console.log('');
  console.log(`Recommendation: ${decision.recommendation}`);
