    ...
    try {
      ...
      if (options.compareStrategies) {
        ...
        if (options.format === 'table') {
          ...
          console.log(
            `  Worst robustness:  ${fri.worst_site} (E-value: ${fri.min_evalue.toFixed(2)})`
          );
          console.log(
            `  Best robustness:   ${fri.best_site} (E-value: ${Math.max(...fri.site_evalues.map((s: SiteEvalue) => s.evalue)).toFixed(2)})`
          );
          console.log();
