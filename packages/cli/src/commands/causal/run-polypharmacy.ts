...
export const runPolypharmacyCommand = new Command('run-polypharmacy')
  ...
  .option('--tier <1|2|3>', 'Interaction tier (1=common 16%, 2=moderate 0.4%, 3=ultra-rare 0.064%)', '3')
  ...
  .action(async (options: RunPolypharmacyOptions) => {
    try {
      ...
      console.log(`  Throughput:              ${((numSites * patientsPerSite) / (totalTime / 1000)).toLocaleString()} patients/sec`);
      console.log('');
