    ...
    try {
      if (options.sitesFile) {
        ...
        if (options.format === 'table') {
          ...
          console.log(`  Point Estimate: ${(decision.mode_distribution.point_estimate * 100).toFixed(0)}%`);
          console.log(`  Bounds:         ${(decision.mode_distribution.bounds * 100).toFixed(0)}%`);
          console.log(`  Sensitivity:    ${(decision.mode_distribution.sensitivity * 100).toFixed(0)}%`);
          console.log();
          
          console.log('Site-Specific Recommendations:');
          console.log('─'.repeat(70));
          for (const [site_id, site_decision] of decision.site_modes.entries()) {
            const indicator = site_decision.mode === 'point-estimate' ? '✓' : 
                            site_decision.mode === 'bounds' ? '⚠' : '!';
            console.log(`  ${indicator} ${site_id.padEnd(15)} → ${site_decision.mode.padEnd(20)} (confidence: ${(site_decision.confidence * 100).toFixed(0)}%)`);
          }
          console.log();
          ...
          const output = {
            ...decision,
            site_modes: Array.from(decision.site_modes.entries()).map(([site_id, mode_decision]) => ({
              site_id,
              ...mode_decision
            }))
          };
        ...
        if (!scores.unconfoundedness_score || !scores.positivity_score || !scores.specification_score) {
          console.error('Error: data-file must contain assumption scores (unconfoundedness_score, positivity_score, specification_score, overall_score)');
          console.error('Tip: First run "harmonia causal diagnose-assumptions" to get these scores');
          process.exit(1);
        }
        ...
        console.error('Error: Must provide either --data-file (single-site) or --sites-file (federated)');
        process.exit(1);
      }
