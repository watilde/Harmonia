...
export function loadSiteData(filePath: string): SiteData {
  ...
  for (const patient of data.patients) {
    if (
      typeof patient.treatment !== 'number' ||
...
export function computeSiteSummary(siteData: SiteData): SiteSummary {
  ...
  const outcome_rate_treated =
    treated.length > 0 ? treated.filter((p) => p.outcome === 1).length / treated.length : 0;

  const outcome_rate_control =
    control.length > 0 ? control.filter((p) => p.outcome === 1).length / control.length : 0;
...
export function printFederatedSummary(federatedData: FederatedData): void {
  ...
  console.log(
    `Total treated: ${federatedData.total_treated} (${((100 * federatedData.total_treated) / federatedData.total_patients).toFixed(1)}%)`
  );
  console.log(
    `Total control: ${federatedData.total_control} (${((100 * federatedData.total_control) / federatedData.total_patients).toFixed(1)}%)`
  );
  console.log('');
  ...
  for (const site of federatedData.sites) {
    ...
    console.log(
      `    Treated: ${summary.n_treated} (${(100 * summary.treatment_rate).toFixed(1)}%)`
    );
    console.log(
      `    Control: ${summary.n_control} (${(100 * (1 - summary.treatment_rate)).toFixed(1)}%)`
    );
    ...
    console.log(
      `    Naive ATE: ${(summary.outcome_rate_treated - summary.outcome_rate_control).toFixed(4)}`
    );
    console.log('');
  }
}
...
export function validateFederatedData(federatedData: FederatedData): {
  ...
  for (const site of federatedData.sites) {
    ...
    if (patients.length !== metadata.n_patients) {
      errors.push(
        `${metadata.site_id}: Patient count mismatch (metadata: ${metadata.n_patients}, actual: ${patients.length})`
      );
    }
    ...
    if (actual_treated !== metadata.n_treated) {
      errors.push(
        `${metadata.site_id}: Treated count mismatch (metadata: ${metadata.n_treated}, actual: ${actual_treated})`
      );
    }

    if (actual_control !== metadata.n_control) {
      errors.push(
        `${metadata.site_id}: Control count mismatch (metadata: ${metadata.n_control}, actual: ${actual_control})`
      );
    }
  }
