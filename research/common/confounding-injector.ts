 ...
 */
export function injectConfounding(patients: Patient[], params: ConfoundingParams): InjectionResult {
  // Validate parameters
...
export function printInjectionSummary(result: InjectionResult): void {
  ...
  console.log(
    `  Treatment rate: ${(result.statistics.confounded_treatment_rate * 100).toFixed(1)}%`
  );
  ...
  console.log(
    `  RR_TU observed: ${result.statistics.observed_RR_TU.toFixed(2)} (target: ${result.params.RR_TU.toFixed(2)})`
  );
  console.log(
    `  RR_YU observed: ${result.statistics.observed_RR_YU.toFixed(2)} (target: ${result.params.RR_YU.toFixed(2)})`
  );
  console.log('');
