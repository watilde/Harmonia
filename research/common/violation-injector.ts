...
export function injectPositivityViolation(
  ...
  const low_ps = severity === 'mild' ? 0.1 : severity === 'moderate' ? 0.05 : 0.02;
  const high_ps = severity === 'mild' ? 0.9 : severity === 'moderate' ? 0.95 : 0.98;
...
export function injectSpecificationViolation(
  ...
  for (const patient of violated) {
    ...
    if (patient.treatment === 1) {
      ...
      const treated_outcome_prob = Math.min(
        0.9,
        Math.max(0.1, baseline_outcome_prob + treatment_benefit)
      );
 ...
 */
export function injectViolation(patients: Patient[], scenario: ViolationScenario): ViolationResult {
...
export function printViolationSummary(result: ViolationResult): void {
  ...
  console.log(
    `Expected Score Range: [${result.scenario.expected_score_range.min.toFixed(2)}, ${result.scenario.expected_score_range.max.toFixed(2)}]`
  );
  console.log('');
  ...
  if (result.violation_stats.covariate_imbalance !== undefined) {
    console.log(
      `  Covariate Imbalance (SMD): ${result.violation_stats.covariate_imbalance.toFixed(3)}`
    );
  }
  if (result.violation_stats.propensity_extremes !== undefined) {
    console.log(
      `  Propensity Extremes:       ${(result.violation_stats.propensity_extremes * 100).toFixed(1)}%`
    );
  }
  if (result.violation_stats.effect_heterogeneity !== undefined) {
    console.log(
      `  Effect Heterogeneity (CV): ${result.violation_stats.effect_heterogeneity.toFixed(3)}`
    );
  }
  console.log('');
  console.log('─'.repeat(70));
