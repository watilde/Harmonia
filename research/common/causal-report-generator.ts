...
export function generateSiteReport(
  ...
  if (mode_decision.mode === 'point-estimate') {
    conclusion = `Based on ${sample_size} patients, all causal assumptions are reasonably satisfied. ` +
    ...
    conclusion = `Based on ${sample_size} patients, some assumption violations detected. ` +
    ...
    conclusion = `Based on ${sample_size} patients, severe assumption violations detected. ` +
...
function generateDetailedFindings(
  ...
  const sites_with_violations = site_reports.filter(r => r.violations.some(v => v.severity !== 'none'));
...
function generateLimitations(
  ...
  if (federated_mode.overall_mode === 'bounds') {
    limitations.push('Partial identification results in wider uncertainty intervals than point estimates');
  }
  
  // Check for small sample sizes
  const small_sites = site_reports.filter(r => r.sample_size < 100);
  if (small_sites.length > 0) {
    limitations.push(`${small_sites.length} sites have small sample sizes (n < 100), limiting statistical power`);
  }
...
function generateRecommendations(
  ...
  if (federated_mode.overall_mode === 'point-estimate') {
    ...
    recommendations.push('Report FRI (Federated Robustness Index) to quantify robustness across sites.');
    recommendations.push('Consider collecting additional covariates to reduce unmeasured confounding.');
    recommendations.push('Strengthen study design (e.g., instrumental variables, randomization) for future iterations.');
  }
...
function printSiteReport(report: SiteReport): void {
  ...
  console.log(`Assumption Scores: Overall=${report.assumption_scores.overall_score.toFixed(2)}, ` +
