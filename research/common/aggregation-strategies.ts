 ...
 */
function computeWeight(n: number, strategy: WeightingStrategy, alpha: number = 0.5): number {
  switch (strategy) {
    case 'sample-size':
      return n;
...
export function aggregateBounds(
  ...
  const weights = siteBounds.map((sb) => computeWeight(sb.bounds.n_total, strategy, alpha));
  ...
  const strategy_name = strategy === 'power' ? `power(α=${alpha})` : strategy;
...
export function formatAggregatedBounds(aggregated: AggregatedBounds): string {
  return (
    `[${aggregated.lower.toFixed(4)}, ${aggregated.upper.toFixed(4)}] ` +
 ...
 */
export function printStrategyComparison(comparisons: Record<string, AggregatedBounds>): void {
  console.log('');
  ...
  const sorted = Object.entries(comparisons).sort((a, b) => a[1].width - b[1].width);
  ...
  console.log(
    `📏 Width reduction: ${(((widest[1].width - narrowest[1].width) / widest[1].width) * 100).toFixed(1)}%`
  );
  console.log('');
}
