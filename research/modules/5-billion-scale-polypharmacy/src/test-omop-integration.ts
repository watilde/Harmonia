...
for (let tier = 1; tier <= 3; tier++) {
  ...
  console.log(
    `  True effect: ${INTERACTION_TIERS[tier - 1].trueEffect > 0 ? '+' : ''}${INTERACTION_TIERS[tier - 1].trueEffect} ml/min/year\n`
  );
  ...
  for (const drugs of drugsByPerson.values()) {
  ...
  console.log(
    `  ✓ Target tier patients: ${targetTierCount} (${(observedPrevalence * 100).toFixed(2)}%)`
  );
  ...
  for (const measurements of measurementsByPerson.values()) {
  ...
  console.log(
    `  ✓ Paired measurements: ${patientsWithPairedMeasurements}/${dataset.persons.length}`
  );
  console.log('');
}
