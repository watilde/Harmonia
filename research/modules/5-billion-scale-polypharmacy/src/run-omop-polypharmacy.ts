...
if (options.federatedAnalysis) {
  ...
  if (options.saveOmop) {
    ...
    for (let siteId = 0; siteId < numSites; siteId++) {
      ...
      if (options.verbose && siteId < 3) {
        console.log(
          `  ✓ Site ${siteId}: ${dataset.persons.length} persons, ${dataset.drugs.length} drugs, ${dataset.measurements.length} measurements`
        );
      }
    }
  ...
  console.log(
    `  Throughput:              ${((numSites * patientsPerSite) / ((Date.now() - startTime) / 1000)).toLocaleString()} patients/sec`
  );
  console.log('');
